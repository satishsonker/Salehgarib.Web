...
export default function BillingTaxTable({ billingData }) {
    ...
    const headers = ["Print", "Sr", "Date", "Invoice No", "Order No.", "Qty", "Total Amount","Total VAT","Gross Amount", "Paid Amount", "Balance Amount",  "Paid VAT", "Balance VAT"];
                        ...
                        {billingData?.map((res, index) => {
                            return <>
                                <tr key={index} style={{ fontSize: '12px' }}>
                                    ...
                                    <td className={calBalVat(res)<0?'bg-danger text-center':'text-center'} style={{ padding: '5px' }} title={calBalVat(res)<0?"Customer has paid more than order VAT":""}>{common.printDecimal(calBalVat(res))}</td>
                                </tr>
