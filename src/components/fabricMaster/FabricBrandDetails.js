...
export default function FabricBrandDetails() {
    ...
    useEffect(() => {
        setIsRecordSaving(true);
        Api.Get(apiUrls.fabricMasterController.brand.getAll + `?PageNo=${pageNo}&PageSize=${pageSize}`).then(res => {
            tableOptionTemplet.data = res.data.data;
            tableOptionTemplet.totalRecords = res.data.totalRecords;
            setTableOption({ ...tableOptionTemplet });
        })
    ...
    const validateError = () => {
        ...
        const newError = {};
        if (!name || name === "") newError.value = validationMessage.fabricBrandNameRequired;
        return newError;
    }
    return (
        <>
                ...
                <div className="modal-dialog">
                    <div className="modal-content">
                                    ...
                                    <div className="card-body">
                                        <form className="row g-3">
                                            <div className="col-md-12">
                                                <Inputbox type="text" labelText="Fabric Brand" isRequired={true} onChangeHandler={handleTextChange} name="name" value={fabricBrandModel.value} className="form-control-sm" errorMessage={errors?.name} />
                                            </div>
                                        </form>
                                    </div>
                                </div>
                        ...
                        <div className="modal-footer">
                            <ButtonBox type={isRecordSaving ? 'Save' : 'Update'} text={isRecordSaving ? 'Save' : 'Update'} onClickHandler={handleSave} />
                            <ButtonBox type="close" modelDismiss={true} modalId="closePopup" />
                        </div>
                    </div>
                    {/* <!-- /.modal-content --> */}
                </div>
