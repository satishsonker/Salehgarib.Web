    ...
    return (
        <>
             <div className="table-actions d-flex align-items-center gap-3 fs-6">
                {option.showPrint && <div style={{ cursor: "pointer" }} onClick={e => option.print.handler(dataId, data)} className="text-success" data-bs-placement="bottom" title={option.print.title} data-bs-original-title={option.print.title} aria-label={option.print?.title} data-bs-toggle="modal" data-bs-target={"#" + (option.print.modelId === undefined ? "" : option.print.modelId)}><i className={option.print.icon}></i></div>}
