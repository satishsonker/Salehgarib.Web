    ...
    return (
        <>
             <div className="table-actions d-flex align-items-center gap-3 fs-6">
                {option.showPrint && <div style={{ cursor: "pointer" }} onClick={e => option.print.handler(dataId, data)} className="text-success" data-bs-placement="bottom" title={option.print.title} data-bs-original-title={option.print.title} aria-label={option.print?.title} data-bs-toggle={option.print.modelId === undefined ? "":"modal"} data-bs-target={"#" + (option.print.modelId === undefined ? "" : option.print.modelId)}><i className={option.print.icon}></i></div>}
                {option.showView && option.view.modelId !== undefined && <div style={{ cursor: "pointer" }} onClick={e => option.view.handler(dataId, data)} className="text-primary" data-bs-placement="bottom" title={option.view?.title} data-bs-original-title={option.view?.title} aria-label={option.view?.title} data-bs-toggle="modal" data-bs-target={"#" + (option.view.modelId === undefined ? "" : option.view.modelId)}><i className={option.view.icon}></i></div>}
                {option.showView && option.view.modelId === undefined &&<div style={{ cursor: "pointer" }} onClick={e => option.view.handler(dataId, data)} className="text-primary" data-bs-placement="bottom" title={option.view?.title} data-bs-original-title={option.view?.title} aria-label={option.view?.title}><i className={option.view.icon}></i></div>}
