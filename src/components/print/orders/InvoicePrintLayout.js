...

                                        {
                                            finalOrder?.map((ele, index) => {
                                                return <tr key={ele.id} className='print-table-height'>
                                                    <td className={"text-center border border-secondary" + getClassName(ele,index)} width="1%" style={{ width: 'max-content !important' }}>{hasKeys(ele) ? (index + 1) + "." : ""}</td>
                                                    <td style={{paddingRight:'0',paddingLeft:'1px'}} className={"text-center border border-secondary text-wrap" + getClassName(ele,index)} width="25%">{getWorkOrderTypes(ele.workType)}</td>
                                                    <td className={"text-center border border-secondary" + getClassName(ele,index)} width="1%">{getModel(ele)}</td>
                                                    <td className={"text-center border border-secondary" + getClassName(ele,index)} width="5%">{common.printDecimal(ele.qty, true)}</td>
                                                    <td className={"text-center border border-secondary" + getClassName(ele,index)} width="10%"><DirhamSymbol amount={common.printDecimal((ele.subTotalAmount / ele.qty), true)}/></td>
                                                    <td className={"text-center border border-secondary" + getClassName(ele,index)} width="10%"><DirhamSymbol amount={common.printDecimal(ele.subTotalAmount, true)}/></td>
                                                    <td className={"text-center border border-secondary" + getClassName(ele,index)} width="10%"><DirhamSymbol amount={common.printDecimal(ele.vatAmount, true)}/></td>
                                                    <td className={"text-center border border-secondary" + getClassName(ele,index)} width="10%"><DirhamSymbol amount={common.printDecimal(ele.totalAmount, true)}/></td>
                                                </tr>
                                            })
                                        }
                                ...
                                <table className='table table-bordered'>
                                    <tbody>
                                        <tr>
                                            ...
                                            <td colSpan={1} className="text-end"><DirhamSymbol amount={common.printDecimal(mainData?.subTotalAmount)}/></td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td className="text-end"><DirhamSymbol amount={common.printDecimal(totalVat)}/></td>
                                            <td className="fs-6 fw-bold text-center">Gross Amount</td>
                                            <td className="text-end"><DirhamSymbol amount={common.printDecimal((mainData?.totalAmount - cancelledOrDeletedTotal))}/></td>
                                        </tr>
                                        <tr>
                                            <td colSpan={6} className="text-start">Received by.................................</td>
                                            <td className="fs-6 fw-bold text-center">Total Advance</td>
                                            <td className="text-end"><DirhamSymbol amount={common.printDecimal(mainData?.accountStatements?.find(x => x.isFirstAdvance)?.credit ?? 0)}/></td>
                                        </tr>
                                        <tr>
                                            <td colSpan={6} className="text-start"></td>
                                            <td className="fs-6 fw-bold text-center">Total Balance</td>
                                            <td className="text-end"><DirhamSymbol amount={common.printDecimal(mainData?.totalAmount - cancelledOrDeletedTotal - (mainData?.accountStatements?.find(x => x.isFirstAdvance)?.credit ?? 0))}/></td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
